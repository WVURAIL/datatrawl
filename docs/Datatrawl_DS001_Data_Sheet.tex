...
\fancyfoot[L]{\small Datatrawl-DS001}
...
\title{datatrawl Storage-Safe Archive Trawling Engine\\Product Specification}
\author{West Virginia University}
\date{Datatrawl-DS001 \\ August 2026}
\hypersetup{
  pdftitle={datatrawl Storage-Safe Archive Trawling Engine -- Product Specification},
  pdfauthor={West Virginia University Radio Astronomy Instrumentation Lab (RAIL)},
  pdfsubject={Datatrawl-DS001},
...
\begin{minipage}[t]{0.39\textwidth}
  ...
  {\small Datatrawl-DS001, August 2026}
\end{minipage}%
...
the datatrawl engine contract for that problem. Datatrail/CADC remains
...
\begin{itemize}
  ...
  \item \textbf{Four-part run model}: a run selects a \field{telescope},
        ...
        \code{datatrawl doctor} checks the selected combination, including the
        reader-to-analyzer stream contract, before a long scan is started.
  ...
  \item \textbf{Persistent inventories}: \code{datatrawl survey} records the
        ...
        \code{datatrawl scan} needs the inventory and analyzer. The built-in
        \code{cadc-datatrail} survey commits per-event rows and completion state transactionally in
        \code{survey_state.sqlite3}; JSONL/text files are generated views. A
        configuration manifest refuses incompatible reuse of the directory.
        Custom sources own their survey persistence.
        The current
...
\textbf{Is not} & A science package. The statistics live in analyzers
              (built-in \code{spectrum}, or external detector plugins). \\
\bottomrule
\end{tabularx}
...
If a source fetch remains active during cancellation, the reported directory
is retained until the writer stops; custom network sources must use bounded
timeouts. \\
...
Resume gate & Fail closed & \field{AccumulatingAnalyzer} persists and validates
analyzer identity, instrument geometry, selection, and
\field{resume\_parameters(ctx)}. A custom \field{resume()} must provide an
equivalent check. \\
...
\code{datatrawl doctor} & Report explicitly compatible combinations; with
...
resume_parameters(ctx)->map  # meaning-changing analyzer options (base class)
...
explicitly at run time with \code{--plugin yourpkg.module}. External
packages register their readers and detector analyzers this way.
...
supplied accumulating base class provides committed-key storage, atomic NPZ
replacement, and a canonical resume manifest. The manifest covers analyzer
identity, instrument geometry, selection, and the normalized mapping returned
by \field{resume\_parameters(ctx)}; its conservative default includes every
run option. Products without the manifest are refused. On restart, the engine
obtains the committed keys through
...
\code{[outriggers]} & \code{hdf5plugin} & Outrigger N\textsuperscript{2}
visibility decode (compression filters). \\
...
3.9--3.14 and runs the offline suite against installed wheel and source
distribution artifacts outside the checkout.
...
\begin{itemize}
  ...
  \item The release job builds the source distribution and wheel, runs
        \code{twine check}, installs each artifact in a new virtual environment,
        runs the complete offline suite from copied/extracted tests, and verifies
        the Datatrail dependency contract.
