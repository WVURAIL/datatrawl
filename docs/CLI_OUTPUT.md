...
- `FAIL fetch` means the file was not staged. The file is not marked done; rerun
  the same command to retry.
- `QUARANTINE` means the bytes were staged and the reader explicitly classified
  a deterministic file/schema problem with `UnreadableUnitError`. The unit
  identity is written to the quarantine ledger and excluded on later runs
  until that ledger entry is removed.
...
With no choices, it prints the run model, combinations whose reader/analyzer stream
contracts explicitly fit, and the command shape to use next. A pair with an incomplete
contract is excluded from this Ready table:
...
| `[--]` | A check could not be completed, usually because an external service was unavailable. The failure is not conclusive. | Read the text after the marker and rerun when the dependency is reachable. |
...
walk. The skipped marker does not identify the cause by itself; the text following
`[--]` identifies which external check could not complete. A missing reader
`stream_kind` or analyzer `accepts_stream_kinds` is instead a validation error and blocks
`doctor`/`scan` until the plugin declares its contract.
...
```text
[survey] chime via cadc-datatrail -> ~/datatrawl-inventories/chime-spectrum-844
[survey] scopes=['chime.event.baseband.raw'] shape=chime-baseband minimum_bytes=1048576 freq_ids=1 (844..844) -> ~/datatrawl-inventories/chime-spectrum-844/inventory.jsonl
to survey: 80 events
...
survey wrote ~/datatrawl-inventories/chime-spectrum-844/inventory.jsonl
  inventory: ~/datatrawl-inventories/chime-spectrum-844/inventory.jsonl
  meta: ~/datatrawl-inventories/chime-spectrum-844/inventory.meta.json
```
...
| `accepted-empty` | The event resolved, but no requested files remained retrievable after the survey's repeated checks, or immediately when the observation is older than `--empty-age-days` (default 30). |
...
The built-in `cadc-datatrail` survey commits each event and its rows together in
`survey_state.sqlite3`. Its `inventory.jsonl`, `surveyed_events.txt`, and other
text ledgers are generated views that may be regenerated after an interruption;
do not hand-edit them or the database. `survey_manifest.json` binds the directory
to its scopes, frequency IDs, reader shape/schema, instrument geometry, and
shape-changing options. Use a fresh `--name` when any input changes. Custom
sources own their persistence behavior.

<p align="center">
  <img src="../assets/fig-survey-states.svg" alt="Survey event lifecycle: an event that yields no usable common path is counted no-data; a resolved event has its expected files verified, producing rows written, or incomplete at the retry limit, or resolved-but-empty, which the next survey retries until the attempt limit or an old observation turns it into accepted-empty." width="840">
</p>
...
done: 5/5 units, 3 new this run, 0 failed | {'count': 15, 'files': 5, 'freq_id': 844, 'peak_sky_mhz': 470.3094}
...
An explicit `UnreadableUnitError` while the reader is streaming is handled
conservatively. The unit is added to the quarantine ledger, but the current
in-memory analyzer state is not saved because it may include a partial update.
Rerun the same command to load the last saved product and exclude the
quarantined unit. Other reader exceptions abort without quarantine.
