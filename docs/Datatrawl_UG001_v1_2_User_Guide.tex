...
\fancyfoot[L]{\small Datatrawl-UG001 v1.2}
...
\date{Datatrawl-UG001 v1.2 \\ July 3, 2026}
...
\begin{minipage}[t]{0.39\textwidth}
  ...
  {\small Datatrawl-UG001 v1.2, July 3, 2026}
\end{minipage}%
...
then processes the archive one file at a time --- fetch, analyze, delete,
checkpoint --- while optionally maintaining an explicitly bounded prefetch
queue. The analyzer may split a selection into one or more independently
resumable products. \code{datatrawl doctor} exists to make the four choices
and their prerequisites legible \emph{before} you spend hours on a scan.
...
file exists at a time. If \code{--max-staged-files} is raised, no more than
that explicitly configured number may be staged. The copies live in a
per-invocation staging directory (concurrent scans cannot delete each other's
files), and each is deleted as soon as the analyzer has consumed it. Do not
point the staging area at precious data.}
