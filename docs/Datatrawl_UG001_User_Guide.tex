...
\fancyhead[C]{\small Storage-Safe Archive Trawling Engine}
\fancyhead[R]{\small User Guide}
\fancyfoot[L]{\small Datatrawl-UG001}
...
\title{datatrawl Storage-Safe Archive Trawling Engine\\User Guide}
\author{West Virginia University}
\date{Datatrawl-UG001 \\ August 2026}
\hypersetup{
  pdftitle={datatrawl Storage-Safe Archive Trawling Engine -- User Guide},
  pdfauthor={West Virginia University Radio Astronomy Instrumentation Lab (RAIL)},
  pdfsubject={Datatrawl-UG001},
...
\begin{minipage}[t]{0.39\textwidth}
  ...
  {\small Datatrawl-UG001, August 2026}
\end{minipage}%
...
datatrawl list telescopes      # ready, geometry-only, or invalid
...
\end{lstlisting}
\code{doctor} reports the installed components, reader-to-analyzer stream
compatibility, configuration, credentials, and readiness of the selected
combination.
...
analysis. It writes a named inventory directory under
\code{~/datatrawl-inventories/} (overridable with
\code{DATATRAWL_INVENTORY_ROOT} or \code{--root}), named
\code{<telescope>-fid<freq_ids>} by default or by \code{--name}. The inventory metadata records the telescope, source, and
...
between commands. The built-in \code{cadc-datatrail} source commits each event and its rows transactionally in
\code{survey_state.sqlite3}; \code{inventory.jsonl} and the text ledgers are
generated views. Do not edit those files or the database. A survey manifest
binds the directory to its scopes, frequency IDs, reader shape, instrument
geometry, and shape-changing options; choose a fresh \code{--name} to change
those inputs or re-probe a completed event. Custom sources own their survey
persistence behavior. During a scan, the current engine loads the rows selected for
...
directory that does not contain source data or other required files. If
cancellation times out while a source fetch is still writing, datatrawl
retains and reports that directory. Wait for the fetch or process to stop
before removing the reported path. Custom network sources must give every
fetch a bounded timeout.}
...
write completes. The accumulating base also stores a fail-closed resume
manifest covering analyzer identity, instrument geometry, selection, and the
analyzer's \field{resume\_parameters(ctx)}. It refuses older products without
that manifest. A custom save/resume format owns equivalent recovery and
compatibility behavior. Use a new output location when intentionally changing
the configuration.
...
The analyzer performs the analysis and owns the product contract. Prefer
\field{AccumulatingAnalyzer}, declare the reader stream it accepts, and register
the class:
\begin{lstlisting}
class MyAnalyzer(AccumulatingAnalyzer):
    ...
    def resume_parameters(self, ctx): # meaning-changing analyzer options
        return {"threshold": float(ctx.options["threshold"])}
    def begin(self, ctx, first_meta): # once, at the first new file
    def consume_file(self, arrays, meta):  # accumulate; return n frames
    def _product(self):               # fields added to the atomic NPZ
    def _restore(self, z):            # restore analyzer accumulators
    def summary(self):                # one status line
\end{lstlisting}
The base class manages committed keys, atomic writes, and the resume manifest.
Its default \field{resume\_parameters} includes every run option; override it
with a normalized, complete mapping when operational paths should not define
product compatibility. A custom \field{save}/\field{resume} implementation
must supply equivalent behavior.
...
\code{docs/DATATRAIL_BOUNDARY.md}.
