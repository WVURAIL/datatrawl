...
\fancyfoot[L]{\small Datatrawl-DS001 v1.2}
...
\date{Datatrawl-DS001 v1.2 \\ July 3, 2026}
...
\begin{minipage}[t]{0.39\textwidth}
  ...
  {\small Datatrawl-DS001 v1.2, July 3, 2026}
\end{minipage}%
...
\section{Features}
\begin{itemize}
  \item Streams telescope archives under an explicit, fixed
        \textbf{local-storage budget}: by default, stage and analyze one file
        at a time; optionally prefetch up to \code{--max-staged-files}; delete
        every staged copy after consumption.
  ...
  \item Atomic product writes (temporary file in the product directory, then
        atomic replace), per-invocation staging directories (concurrent scans
        cannot delete each other's files), and unit-key provenance embedded
        in every product.
...
\toprule
\textbf{Is} & A run layer that walks an archive inventory, processes one file
              at a time, hands its arrays to a science analyzer, saves a
              resumable product, and deletes each staged copy after use. The
              source may maintain an explicitly bounded prefetch queue. \\
...
\node[soft,below=of src] (stage) {Staging dir\\{\scriptsize bounded queue; deleted after use}};
\draw[->,thick] (arch) -- (src);
\draw[->,thick] (src) -- (rdr);
\draw[->,thick] (rdr) -- (ana);
...
Product write & Atomic & a temporary file in the product directory, then atomic replace; a kill never leaves
a partial product. \\
...
v1.2 & July 3, 2026 & Version designation: engine v0.2.0 re-tagged v1.0.0
