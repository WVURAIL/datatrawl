...
setups. Save the new file as `src/datatrawl/instruments/<name>.yaml` in a
source checkout (`pip install -e .`): telescopes are discovered only from the
package's `instruments/` directory.
...
- **`geometry-only`** -- the geometry and Nyquist zone are set, but `scopes` is empty. The
  ...
  supplied explicitly.
- **`invalid`** -- a required field is missing or malformed, an inherited configuration
  cannot be resolved, or the file's configured name does not match its filename. The
  listing includes the validation problem; correct the YAML before running it.
