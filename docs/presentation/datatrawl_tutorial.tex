...
% -----------------------------------------------------------------------------
% Theme: Amurmaple with the WVU brand color theme.
...
\input{../../assets/datatrawl-figures.sty}
...
\webpage{wvurail.org}
\titlegraphic{\includegraphics[width=0.45\textwidth]{imgs/gwac.png}\\[6ex]}
\logo{\;\includegraphics[width=1.15cm]{imgs/wv.png}}
...
\begin{frame}[plain]
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=south west,text=white!75!DTCyanPale,font=\fontsize{6.3}{7.3}\selectfont,
      ...
      {Repository: \href{https://github.com/WVURAIL/datatrawl}{WVURAIL/datatrawl}.};
  \end{tikzpicture}}
  \note{
...
\begin{frame}{Without Datatrawl, the user is the workflow engine}
  ...
  \note{
    The motivating workload was a DTV pilot detector: apply the same F-statistic detector
...
\begin{frame}{Weeks-long jobs will be interrupted}
  ...
  \source{\href{https://github.com/WVURAIL/datatrawl/blob/master/docs/TROUBLESHOOTING.md}{docs/TROUBLESHOOTING.md}: long runs can outlive a terminal, certificate, or compute session.}
  \note{
...
\begin{frame}{Datatrail maps the archive; Datatrawl runs the analysis}
  ...
  \source{Repository boundary statement: ``datatrawl should not try to become Datatrail'' --- \href{https://github.com/WVURAIL/datatrawl/blob/master/docs/DATATRAIL_BOUNDARY.md}{DATATRAIL\_BOUNDARY.md}.}
  \note{
...
\begin{frame}{Discovery and execution are separate phases}
  ...
  \source{Workflow and artifact names: repository README and CLI help at the revision used to build this deck.}
  \note{
...
\begin{frame}{One fixed engine connects four interchangeable pieces}
  \input{../../assets/architecture.tikz}
...
\begin{frame}{Most pieces already exist; you write the analyzer}
  \input{../../assets/fig-plugin-ecosystem.tikz}
  ...
  \source{Registered plugins: \texttt{datatrawl list}; plugin design: repository README.}
  \note{
    ...
    cadc-datatrail and local today, perhaps cluster-filesystem sources
    someday --- and readers whenever a new data type appears;
    baseband, gain, and N-squared ship today. The analyzer is the deliberate
...
\begin{frame}{Survey is a filter: 23 channels across selected CHIME scopes}
  \input{../../assets/fig-survey-filter.tikz}
  ...
  \note{
    Survey is a filter over the archive. The motivating campaign's criteria say: across
...
% 12
\begin{frame}{A second campaign: same survey, different criteria}
  \begin{tikzpicture}[x=.92cm,y=.92cm]
    ...
    % the same funnel, different criteria
  ...
  \note{
    A second campaign asks the opposite question: keep every channel, but only
...
\begin{frame}{Scan is a pull--process--delete conveyor}
  \input{../../assets/fig-scan-conveyor.tikz}
...
\begin{frame}{The scratch bound is explicit}
  \input{../../assets/fig-scratch-bound.tikz}
...
\begin{frame}{Checkpoints turn completed units into durable progress}
  \input{../../assets/fig-checkpoint-resume.tikz}
...
\begin{frame}{The analyzer contains the science --- and the resume contract}
  ...
  \source{Analyzer guide: \href{https://github.com/WVURAIL/datatrawl/blob/master/docs/ADDING_AN_ANALYZER.md}{docs/ADDING\_AN\_ANALYZER.md}; base class: \texttt{analyzer\_base.py}.}
  \note{
...
\begin{frame}{Your analyzer starts from working code}
  \begin{tikzpicture}[x=.92cm,y=.92cm]
    ...
    \node[text=DTSoft,font=\fontsize{10}{12.5}\selectfont,align=left] at (4.0,2.2)
      {\checkmark\ a working analyzer: \texttt{freq\_id-peak}\\[0.6mm]
       ...
       \checkmark\ immutable public core baseline};
  ...
  \source{\href{https://github.com/WVURAIL/datatrawl-analyzer-template}{WVURAIL/datatrawl-analyzer-template}: \texttt{freq\_id-peak} example, entry-point registration, engine smoke tests, immutable core baseline.}
  \note{
    ...
    as the science outgrows the tone. The immutable datatrawl commit in
    pyproject is the current public integration baseline: advance it to the
    fixed release deliberately and rerun the tests.
  }
\end{frame}

% 21
\begin{frame}{For the motivating detector, the machinery is already reusable}
  \begin{tikzpicture}[x=.92cm,y=.92cm]
    ...
    \node[cardamber,minimum width=2.75cm,minimum height=1.5cm,text width=2.35cm,inner xsep=2mm] (ana) at (11.38,3.15)
      {\bfseries F-statistic\\\fontsize{8.5}{10.5}\selectfont detector analyzer};
  ...
  \note{
    This is the payoff for the motivating detector campaign. The CHIME
    instrument geometry, the CADC/Datatrail source, and the CHIME baseband reader
    already exist. The project supplies the F-statistic analyzer as a plugin and
    owns the scientific method. The analyzer can plan the 23
...
\begin{frame}{Everything before survey runs without credentials}
  \begin{tikzpicture}[x=.92cm,y=.92cm]
    ...
    \node[text=DTSoft,font=\fontsize{10.2}{12.5}\selectfont,align=left] at (3.35,2.8)
      {\checkmark\ \texttt{datatrawl} installed\\[1mm]
...
\end{lstlisting}
  ...
  \note{
    ...
    readers, and analyzers, including external plugins. Doctor checks the
    declared reader-to-analyzer stream contract and whether a requested
    combination is ready. The offline test suite exercises reader,
...
\end{lstlisting}
  ...
  \source{\texttt{--name} resolves under \texttt{\$DATATRAWL\_INVENTORY\_ROOT} or \texttt{\string~/datatrawl-inventories/} by default;\\explicit \texttt{--out} still wins.}
  \note{
    ...
    The name becomes the stable handle used by explore and scan. Named
    inventories are written below DATATRAWL-INVENTORY-ROOT, or the
...
\begin{frame}{The operator stops being the workflow engine}
  ...
  \source{Tutorial grounded in the reviewed \href{https://github.com/WVURAIL/datatrawl}{WVURAIL/datatrawl} source revision.}
  \note{
    ...
    while the science project owns the analyzer and its
...
\begin{frame}{Rerun this tutorial, then bring your science}
  ...
  \note{
    ...
    backup slides are derived from the repository contracts; re-check them
...
\begin{frame}{Backup: where durable state lives}
  ...
  \note{
    Named inventories are independent of the working directory.
...
\begin{frame}{Backup: evidence base}
  \begin{tikzpicture}[x=.92cm,y=.92cm]
    \node[terminal,minimum width=14.2cm,minimum height=4.2cm] at (7.83,2.65) {\begin{minipage}{13.5cm}
      ...
      \href{https://github.com/WVURAIL/datatrawl}{github.com/WVURAIL/datatrawl}, reviewed source revision\par\smallskip
  ...
  \note{
    The deck is grounded in the live repository shown here. The implementation
