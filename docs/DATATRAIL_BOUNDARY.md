...
## Verdict taxonomy of `Datatrail.files()` / `common_path()`

Three outcomes, never conflated:

1. **Transient** -- the CLI/service did not answer (no payload, or the
   `{"error": ...}` envelope). Returned as the not-answered verdict and
   retried with backoff; a sustained run of these is the signature of an
   expired certificate or a real outage.
2. **Deterministic refusal** -- the service answered, but the payload is not
   one this adapter can act on: schema drift (missing/non-dict `files`),
   malformed replica lists, a minoc replica outside `_MINOC_COLLECTIONS`, or
   no usable common-path split. Raised as `DatatrailContractError` with the
   offending detail; the survey records the event as `refused` in the
   accept-as-empty ledger and moves on. Retrying a deterministic answer is
   how one odd event stalls -- and then aborts -- a ten-thousand-event
   survey.
3. **No data** -- a clean answer with no minoc files. The event enters the
   accepted-empty lifecycle.
