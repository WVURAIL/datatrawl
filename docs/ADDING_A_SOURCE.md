...
class MySource(DataSource):
    ...
    def fetch(self, unit: Unit, dest: str):
        try:
            my_download(
                unit.meta["fetch_uri"], dest,
                timeout=FETCH_TIMEOUT_SECONDS,
            )
            return True, ""
        except (TimeoutError, ConnectionError) as exc:
            # An expected, file-level failure: logged now and retried when the
            # scan is rerun. Add only the documented retryable exceptions from
            # the client library used by this source.
...
The return contract is exact: success is `(True, detail: str)`, and an expected
retryable/file-level failure is `(False, detail: str)`. Both elements must be a real
`bool` and `str`. Catch only the client exceptions that establish that expected
disposition. Any other exception, or a malformed return value, is a run-level source
failure; the engine aborts so an implementation or environment bug is not mislabeled as
one bad file.

`fetch()` runs in a downloader thread, and Python cannot forcibly stop a network call that
is blocked inside a client library. Configure finite connect, read, and/or overall
timeouts on every remote operation, as the example's named policy constant illustrates.
Choose a limit appropriate for the archive and object size, and return only after the
destination file is closed; do not start an untracked background writer.

When another part of a scan fails, the engine signals the downloaders to stop and waits a
brief bounded interval. If `source.fetch()` is still active, it raises
`datatrawl.pipeline.ActiveDownloadWorkersError` and retains the reported scratch directory
rather than deleting a path beneath a live writer. A bounded source timeout ensures that
operation eventually returns. See [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) for safe
cleanup of the retained directory.
...
`scan` commands all accept repeated `--set key=value` arguments. Read them directly from
`ctx.options`. An explicit `none`/`null` remains `None`, and the CLI rejects keys reserved
for core engine settings rather than letting a plugin overwrite them.
