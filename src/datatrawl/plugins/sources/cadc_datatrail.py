...
A source for CANFAR archive work, in three halves:
  ...
  enumerate()  read an inventory (inventory.jsonl) and yield one Unit per file,
               ...
               `datatrawl.selection` for the grammar). The inventory is a cheap,
...
from ._cadc_inventory import (
    annotate_row, candidate_file, join_uri, logical_unit_key, parse_row,
)
from . import _cadc_transport
from ._datatrail import DATATRAIL, Datatrail, DatatrailContractError
from ._datatrail_recon import match_terms, recon
from ._survey_state import (
    ...
    def survey(self, ctx: RunContext, out_dir: str) -> str:
        ...
        try:
            for i, (scope, ev) in enumerate(events, 1):
                ...
                if status == "refused":
                    ...
                    # widen _MINOC_COLLECTIONS in _datatrail.py and re-open
        ...
        if n_refused:
            print("contract refusals name the offending replica URI in the "
                  ...
                  "_MINOC_COLLECTIONS in plugins/sources/_datatrail.py and "
