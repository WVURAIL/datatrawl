...
REQUIRED_KEYS = {
    "analysis", "product_schema", "psd", "psd_sum", "count", "freqs_hz",
    "freqs_sky_hz", "f_center_hz", "freq_id", "nfft", "fs_hz",
    "nyquist_zone", "files", "unit_keys", "created",
}
...
def main() -> int:
    ...
    if "analysis" not in keys or str(z["analysis"]) != "spectrum":
        ...
        ok = False
    if str(z["product_schema"]) != "datatrawl.spectrum/v1":
        _fail("product has the wrong spectrum product_schema")
        ok = False
