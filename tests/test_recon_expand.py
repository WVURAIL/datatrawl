...
dtcli internals by hand. --expand closes that gap inside recon's charter:
  ...
  * a dataset that yields no children keeps its own row (the adapter's []
    ...
    silently dropped);
  * without --expand, rows use the compact {scope, dataset} schema;
...
def fake_landscape(down=()):
    ...
    saved_installed = datatrail_cls.__dict__["installed"]
    saved_api_available = datatrail_cls.__dict__["api_available"]
    for n, f in fakes.items():
        setattr(tgt, n, f)
    datatrail_cls.installed = staticmethod(lambda: True)
    datatrail_cls.api_available = staticmethod(lambda: (True, ""))
    try:
        yield
    finally:
        datatrail_cls.installed = saved_installed
        datatrail_cls.api_available = saved_api_available
...
def test_without_expand_uses_compact_schema_and_guidance(tmp_path):
    rows, out = _recon(tmp_path, match="gain")
    assert all(set(r) == {"scope", "dataset"} for r in rows)
...
def test_events_in_dataset_returns_event_ids(tmp_path):
    with fake_landscape():
