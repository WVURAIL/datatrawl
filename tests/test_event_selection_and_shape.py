...
use case: beamforming every freq_id of ONE event into a single product):
  ...
  1. SELECTION -- `ctx.selection` carries events as well as freq_ids, via one
     shared grammar (`datatrawl.selection`). Supported freq_id forms remain
     unambiguous; an event filter is always explicit
  ...
  2. SHAPE -- the archive file shape (which files one event contributes, and
     ...
     re-derivation, no drift. Rows without the required `name` are rejected
     with re-survey guidance rather than guessed at scan time.
...
import datatrawl.selection as selection_module
from datatrawl.interfaces import Reader, RunContext, PluginInfo, READY
from datatrawl.plugins.sources import cadc_datatrail
from datatrawl.selection import parse_freq_ids, parse_selection
from datatrawl.plugins.sources.local import LocalDirectorySource
from datatrawl.plugins.readers.chime_baseband import (
    MINIMUM_ARCHIVE_BYTES, baseband_filename)
...
def test_parse_selection_fails_loud():
    ...
    with pytest.raises(SystemExit, match="selection dict is empty"):
        parse_selection({})
    with pytest.raises(SystemExit):
    ...
    for nested in ({"events": {}}, {"events": [[EV_A]]}):
        with pytest.raises(SystemExit):
            parse_selection(nested)


def test_freq_id_bounds_are_instrument_aware():
    assert parse_selection("0-3", n_channels=4).freq_ids == {0, 1, 2, 3}
    for bad in ("-1", "4", "2-4"):
        with pytest.raises(SystemExit, match="freq_id"):
            parse_selection(bad, n_channels=4)


@pytest.mark.parametrize("n_channels", [0, -1, 4.0, "4", True])
def test_freq_id_geometry_requires_a_positive_integer(n_channels):
    with pytest.raises(SystemExit, match="invalid n_channels"):
        parse_freq_ids("0", n_channels=n_channels)


def test_combined_ranges_share_one_cardinality_limit(monkeypatch):
    monkeypatch.setattr(selection_module, "_MAX_SELECTION_CARDINALITY", 5)
    assert parse_selection("0-2,2-4").freq_ids == {0, 1, 2, 3, 4}
    with pytest.raises(SystemExit, match="too many channels"):
        parse_selection("0-2,4-6")


def test_event_selection_shares_the_cardinality_limit(monkeypatch):
    monkeypatch.setattr(selection_module, "_MAX_SELECTION_CARDINALITY", 2)
    assert parse_selection("events:A,A,B").events == {"A", "B"}
    with pytest.raises(SystemExit, match="too many events"):
        parse_selection("events:A,B,C")
...
def test_reversed_range_is_loud_not_select_all():
    # A descending range must not become an empty "no filter" selection that
    # turns a one-channel scan into an archive pull.
    with pytest.raises(SystemExit) as ei:
...
# --------------------------------------------------------------------------
# 2) archive enumerate: filters + strict self-describing rows
# --------------------------------------------------------------------------
...
def test_enumerate_uses_verified_baseband_name(mixed_inventory):
    (u,) = [u for u in _units(mixed_inventory, None) if u.meta.get("freq_id") == 614]
    assert u.name == baseband_filename(EV_A, 614)
    assert u.meta["quarantine_key"] == u.key
    assert json.loads(u.key.split(":", 1)[1]) == ["s", EV_A, u.name]
    assert u.meta["cadc_uri"] == f"{CP}/{u.name}"


def test_enumerate_rejects_row_without_name(tmp_path):
    inv = _write_inventory(str(tmp_path), [{
        "scope": "s", "event": EV_A, "freq_id": 614, "size_bytes": 10,
        "common_path": CP,
    }])
    ...
    assert f"{inv}:1" in message and "name" in message
...
def test_enumerate_row_name_is_authoritative(mixed_inventory):
    ...
    assert u.meta["cadc_uri"] == f"{CP}/gains_{EV_B}.h5"
    # Shape-specific columns ride through meta untouched; source identity remains
    # the explicit logical (scope, event, name) key when there is no freq_id.
...
def test_enumerate_event_and_freq_filters(mixed_inventory):
    ...
    # no filter returns every current-schema row
...
def fake_archive(events):
    ...
    cadc_datatrail.DATATRAIL.common_path = lambda scope, ev, **kwargs: (
        f"cadc:TEST/data/raw/2020/01/01/{ev}", True)
    cadc_datatrail.CadcDatatrailSource._cadc_size = (
        lambda self, uri, *a, **k: (MINIMUM_ARCHIVE_BYTES + 1, None))
    try:
        yield
    finally:
...
def test_survey_uses_reader_shape_and_roundtrips(tmp_path):
    ...
    assert json.loads(u.key.split(":", 1)[1]) == ["s", EV_A, r["name"]]
    assert u.meta["cadc_uri"].endswith(f"/{r['name']}")
