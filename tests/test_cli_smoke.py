...
EXPECTED = {
    "cadc-datatrail", "local", "chime-baseband", "outrigger-gains",
    "outrigger-n2", "spectrum",
}
...
def test_doctor_runs_without_error():
    r = _run("doctor")
    # The no-choice overview is static discovery and always returns zero. What
    # must NOT happen is an unhandled exception.
    assert r.returncode == 0, f"unexpected doctor exit {r.returncode}"
